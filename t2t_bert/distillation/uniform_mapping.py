...
def l2_distance(source_prob, target_prob):

	source_prob = tf.nn.log_softmax(source_prob)
	target_prob = tf.nn.log_softmax(target_prob)

	l2_distance = tf.sqrt(tf.reduce_sum(tf.pow(source_prob-target_prob, 2.0), axis=(1)))
...
def attention_score_matching(teacher_score, student_score,
	...
	for i in range(len(student_score)):
		...
		for j in range(len(teacher_score)):
			...
			loss += weight*l2_distance(teacher_score_, student_score_)
