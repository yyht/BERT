...
try:
	...
	from .token_discriminator import discriminator_metric_train, discriminator_metric_eval, optimal_discriminator
	...
	from .generator_as_discriminator import model_fn_builder as discriminator_generator
	from .token_discriminator import modified_loss
except:
	from discriminator_gumbel import model_fn_builder as discriminator
	from generator import model_fn_builder as generator
	from generator_gumbel_normal import model_fn_builder as generator_normal
...
def classifier_model_fn_builder(
	...
	def model_fn(features, labels, mode, params):
		...
		discriminator_fn = discriminator_generator(model_config_dict['discriminator'],
		...
		[disc_loss, 
		disc_logits, 
		disc_per_example_loss] = optimal_discriminator(model_config_dict['discriminator'],
									generator_dict, 	
									features,
									discriminator_dict, 
									discriminator_features,
		...
		not_equal_loss_all, 
		not_equal_loss_self] = modified_loss(disc_per_example_loss, 
											disc_logits, 
											discriminator_features['input_ori_ids'], 
											discriminator_features['ori_input_ids'], 
											discriminator_features['input_mask'],
											sampled_binary_mask=discriminator_features.get('sampled_binary_mask', None),
											**kargs)
		output_dict = {}
		...
		if mode == tf.estimator.ModeKeys.TRAIN:
			...
			if not kargs.get('use_tpu', False):
				metric_dict = discriminator_metric_train(output_dict['per_example_loss'],
								output_dict['logits'], 
			...
			train_op = get_train_op(generator_dict, output_dict, optimizer_fn, opt_config,
