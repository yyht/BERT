	...
	def build_embedder(self, input_ids, token_type_ids, 
		...
		with tf.variable_scope(self.config.get("scope", "bert"), reuse=reuse):
			with tf.variable_scope("embeddings"):
				...
				(self.embedding_output_word, self.embedding_table) = bert_seq_modules.embedding_lookup(
						input_ids=input_ids,
	...
	def build_encoder(self, input_ids, input_mask, 
		...
		with tf.variable_scope(self.config.get("scope", "bert"), reuse=reuse):
			with tf.variable_scope("encoder"):
				...
				if kargs.get('seq_type', None) is not None:
					attention_mask = bert_utils.generate_seq2seq_mask(attention_mask, 
														kargs.get('mask_sequence', None),
														kargs.get('seq_type', None),
														**kargs)
		...
		with tf.variable_scope(self.config.get("scope", "bert"), reuse=reuse):
			...
			with tf.variable_scope("pooler"):
				...
				self.pooled_output = tf.layers.dense(
						...
						activation=tf.tanh,
						kernel_initializer=bert_seq_modules.create_initializer(self.config.initializer_range))
	
	def get_present(self):
		return tf.stack(self.all_present, axis=2) # cache internal states
		...
		with tf.variable_scope(self.config.get("scope", "bert"), reuse=reuse):
			with tf.variable_scope("encoder"):
				...
				self.all_attention_scores] = bert_seq_modules.transformer_model(
						...
						do_return_all_layers=True,
						past=past)
				self.cached_present = tf.stack(self.all_present, axis=)
