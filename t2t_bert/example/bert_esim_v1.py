...
def bert_lstm_encoding(model_config, features, labels, 
			mode, target, max_len,
			scope, dropout_rate, 
			reuse=None):
	...
	first_input_mask = input_mask[:,1:max_len+1]
	first_layer_repres = layer_repres[:,1:max_len+1,:]
	
	second_input_mask = input_mask[:,max_len+2:-1]
	second_layer_repres = layer_repres[:,max_len+2:-1,:]

	return [first_input_mask, first_layer_repres,
			second_input_mask, second_layer_repres]
...
def bert_multihead_pooling(model_config, repres, repres_mask, 
					scope, dropout_rate, reuse=None):
...
def classifier_attn_model_fn_builder(
	...
	def model_fn(features, labels, mode):
		...
		a_repres = lstm_model(model_config, 
							a_repres, 
							a_mask, 
							dropout_prob, 
							scope, model_reuse)

		b_repres = lstm_model(model_config, 
							b_repres, 
							b_mask, 
							dropout_prob, 
							scope, True)
		...
		repres_a = bert_multihead_pooling(model_config, a_output, 
					a_mask, 
					scope, 
					dropout_prob, 
					reuse=model_reuse)

		repres_b = bert_multihead_pooling(model_config, b_output,
					b_mask,
					scope, 
					dropout_prob,
					reuse=True)
