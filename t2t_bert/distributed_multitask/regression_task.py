...
def model_fn_builder(model,
	...
	def model_fn(features, labels, mode):
		...
		logits = tf.reduce_sum(feature_a*feature_b, axis=-1)
		per_example_loss = tf.pow(logits - tf.cast(label_ids, dtype=tf.float32), 2)
		print(per_example_loss.get_shape(), '==regression shape==')
		loss_mask = tf.cast(features["{}_loss_multipiler".format(task_type)], tf.float32)
