mpirun -np 2 \
 -H localhost:2 \
 python ./t2t_bert/distributed_bin/hvd_train_eval_api.py \
 --buckets "/data/xuht" \
 --config_file "./data/sentence_embedding/textlstm/textlstm.json" \
 --init_checkpoint "" \
 --vocab_file "sentence_embedding/data/char_id.txt" \
 --label_id "sentence_embedding/cluster_corpus_label_dict.json" \
 --max_length 128 \
 --train_file "sentence_embedding/data/train_tfrecords" \
 --dev_file "sentence_embedding/data/dev_tfrecords" \
 --model_output "sentence_embedding/data/model/textlstm_0329/" \
 --epoch 100 \
 --num_classes 165579 \
 --train_size 827895 \
 --eval_size 827895 \
 --batch_size 32 \
 --model_type "textlstm" \
 --if_shard 1 \
 --is_debug 1 \
 --run_type "sess" \
 --opt_type "hvd" \
 --num_gpus 4 \
 --parse_type "parse_batch" \
 --rule_model "normal" \
 --profiler "no" \
 --train_op "adam" \
 --running_type "train" \
 --cross_tower_ops_type "paisoar" \
 --distribution_strategy "MirroredStrategy" \
 --load_pretrained "no" \
 --w2v_path "w2v/tencent_ai_lab/char_w2v.txt" \
 --with_char "no_char" \
 --input_target "a" \
 --decay "no" \
 --warmup "no" \
 --distillation "normal" \
 --temperature 2.0 \
 --distillation_ratio 0.5 \
 --task_type "single_sentence_classification" \
 --classifier order_classifier


