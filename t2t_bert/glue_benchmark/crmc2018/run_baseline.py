...
# tf.flags.DEFINE_string("exclude_scope", "", "[Optional] TensorFlow master URL.")
# tf.flags.DEFINE_string("model_type", "bert", "[Optional] TensorFlow master URL.")
# tf.flags.DEFINE_string("attention_type", "normal_attention", "[Optional] TensorFlow master URL.")
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if init_checkpoint:
      ...
      ) = modeling.get_assignment_map_from_checkpoint(tvars, init_checkpoint, **kargs)
      if use_tpu:
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      train_op = optimization.create_optimizer(
          total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu, **kargs)
...
def main(_):
  ...
  # do predictions
  if FLAGS.do_predict:
    predict_file = os.path.join(FLAGS.buckets, FLAGS.predict_file)
    eval_examples = read_squad_examples(
        input_file=predict_file, is_training=False)

    eval_writer = FeatureWriter(
        filename=os.path.join(output_dir, "predict.tf_record"),
        is_training=False)
    eval_features = []
    ...
    output_prediction_file = os.path.join(output_dir, output_json_name)
    output_nbest_file = os.path.join(output_dir, output_nbest_name)
  ...
  # do predictions
  if FLAGS.do_eval:
    eval_file = os.path.join(FLAGS.buckets, FLAGS.eval_file)
    eval_examples = read_squad_examples(
