...
from utils.reformer.reformer_utils import sort_key_val, batched_index_select, make_unit_length, chunked_sum, process_inputs_chunk

import tensorflow as tf

class TFLSHAttention(object):
    def __init__( self,
                  dropout = 0.,
                  bucket_size = 64,
                  n_hashes = 8,
    ...
    def call(self, qk, v):
        ...
        class UnsortLogits(object):
            def __init__(self):
                super(UnsortLogits, self).__init__()
            
            def __call__(self, so, slogits):
    ...
    def __call__(self, qk, v):
    	return self.call(qk, v)

class TFLSHSelfAttention(object):
