...
def nonnegative_softmax_kernel_feature_creator(data,
  ...
  data_shape = bert_utils.get_shape_list(data)
  projection_matrix_shape = bert_utils.get_shape_list(projection_matrix)
  if normalize_data:
    # We have e^{qk^T/sqrt{d}} = e^{q_norm k_norm^T}, where
    # w_norm = w * data_normalizer for w in {q,k}.
    data_normalizer = 1.0 / (tf.sqrt(tf.sqrt(data_shape[-1])))
  else:
    data_normalizer = 1.0
  ratio = 1.0 / tf.sqrt(projection_matrix_shape[0])
  data_mod_shape = data_shape[0:len(batch_dims_t)] + projection_matrix_shape
  data_thick_random_matrix = tf.zeros(data_mod_shape) + projection_matrix

  # data_dash = tf.dot_general(
  #     data_normalizer * data,
  #     data_thick_random_matrix,
  #     (((data.ndim - 1,), (data_thick_random_matrix.ndim - 1,)),
  #      (batch_dims_t, batch_dims_t)),
  #     precision=precision)
  data_dash = tf.matmul(data_normalizer * data, 
                      data_thick_random_matrix, 
                      transpose_b=True)

  data_dash_shape = bert_utils.get_shape_list(data_dash, expected_rank=[4])

  diag_data = tf.square(data)
  diag_data = tf.reduce_sum(diag_data, axis=data.ndim - 1)
  diag_data = (diag_data / 2.0) * data_normalizer * data_normalizer
  diag_data = tf.expand_dims(diag_data, axis=data.ndim - 1)

  if is_query:
    last_dims_t = (len(data_dash_shape) - 1,)
    data_dash = ratio * (
        tf.exp(data_dash - diag_data -
                tf.reduce_max(data_dash, axis=last_dims_t, keepdims=True)) + eps)
  else:
    data_dash = ratio * (
...
def sincos_softmax_kernel_feature_creator(data,
  ...
  data_dash = tf.matmul(data_normalizer * data, data_thick_random_matrix, transpose_b=True)


  data_dash_cos = ratio * tf.cos(data_dash)
  data_dash_sin = ratio * tf.sin(data_dash)
  data_dash = tf.concatenate((data_dash_cos, data_dash_sin), axis=-1)
  ...
  data_renormalizer = tf.reduce_max(diag_data, attention_dims_t, keepdims=True)
...
class GaussianUnstructuredRandomMatrix(RandomMatrix):
  ...
  def get_2d_array(self):
    ...
    return tf.random_normal((self.nb_rows, self.nb_columns))
