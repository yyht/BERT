...
def mish(input_tensor):
	modulation = tf.nn.tanh(tf.nn.softplus(input_tensor))
...
def embedding_lookup_factorized(input_ids, # Factorized embedding parameterization provide by albert
	...
	if hidden_size != embedding_size:
		...
		# output *= 2 # this is needed to make value of matrix is consistent with origin bert
	else:
		project_variable = None
		output = output_middle

	# reshape back to 3 rank
	input_shape = bert_utils.get_shape_list(input_ids)  # input_shape=[ batch_size, seq_length, 1]
...
def transformer_model(input_tensor,
	...
	if input_width != hidden_size:
		input_tensor = tf.layers.dense(
							...
							hidden_size,
							kernel_initializer=create_initializer(initializer_range))
		print("===do embedding matrix projections===")
