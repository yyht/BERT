...
def get_sinlge_instance(title, comment, tokenizer, max_length=256):
	...
	_truncate_seq_pair_v1(tokens_a, tokens_b, max_num_tokens, rng)
	...
	input_ids = tokenizer.padding(input_ids, max_length, 0)
	input_mask = tokenizer.padding(input_mask, max_length, 0)
	segment_ids = tokenizer.padding(segment_ids, max_length, 0)
...
infer_api = SavedModelInfer(model_config)
...
import time
start = time.time()
features = get_sinlge_instance(title, comment, tokenizer, max_length=256)
resp = infer_api.infer(features)
print(resp)
