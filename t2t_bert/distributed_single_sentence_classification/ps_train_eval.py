...
def monitored_sess(FLAGS,
	...
	if worker_count >= 1 and FLAGS.opt_type == "ps_sync" or FLAGS.opt_type == "ps":
		...
		available_worker_device = "/job:worker/task:%d/gpu:%d" % (task_index, 0)
		print("==worker device==", available_worker_device, "==task_index==", task_index)
		print("==cluster==", cluster, "==target==", target)
