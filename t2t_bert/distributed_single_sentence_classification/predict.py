...
class InferAPI(object):
	...
	def init_model(self):
		...
		with self.graph.as_default():
			...
			self.opt_config = Bunch({"train_op":"adam"})
			self.model_io_config = Bunch({"fix_lm":False})

			if self.config.get("rule_model", "rule"):
				model_fn_interface = rule_model_fn_builder
				self.rule_config =  self.config["rule_config"] 
				self.rule_api = rule_detector.RuleDetector(self.rule_config)
				self.rule_api.load(self.tokenizer)
			else:
				model_fn_interface = model_fn_builder

			# no matter what output type, for prediction, it always return predict 
			self.model_fn = model_fn_interface(self.config, self.num_classes, init_checkpoint, 
												model_reuse=None, 
