...
flags.DEFINE_integer("max_seq_length", 300, "Maximum sequence length.")
...
def create_instances_from_document(
	...
	random_document_lst = random.sample(index_range, len(document))
		...
		if i == len(document) - 1 or current_length >= target_seq_length:
			if current_chunk:
				...
				if len(current_chunk) == 1 or rng.random() < 0.5:
					...
					#     break
					random_document_index = random_document_lst[i]
...
def create_masked_lm_predictions(tokens, masked_lm_prob,
	...
	for (i, token) in enumerate(tokens):
		...
		# else:
		# 	cand_indexes.append([i])
		cand_indexes = whole_word_mask(cand_indexes, token)
