python ./t2t_bert/distributed_data_prepare/classification_distillation_data_prepare.py \
	--buckets /data/xuht \
	--train_file chenxi/disu/train_disu.txt \
	--dev_file chenxi/disu/dev_disu.txt \
	--test_file chenxi/disu/dev_disu.txt \
	--train_result_file chenxi/disu/data/distillation/data/train_tfrecords \
	--dev_result_file chenxi/disu/data/distillation/data/dev_tfrecords \
	--test_result_file chenxi/disu/data/distillation/data/test_tfrecords \
	--supervised_distillation_file chenxi/disu/distillation/train_distillation.info \
	--unsupervised_distillation_file chenxi/disu/distillation/dev_distillation.info \
	--vocab_file ./data/chinese_L-12_H-768_A-12/vocab.txt \
	--label_id ./data/lcqmc/label_dict.json \
	--lower_case False \
	--max_length 128 \
	--if_rule "no_rule" \
	--rule_word_dict /data/xuht/porn/rule/rule/phrases.json \
	--rule_word_path /data/xuht/porn/rule/rule/mined_porn_domain_adaptation_v2.txt \
	--rule_label_dict /data/xuht/porn/rule/rule/rule_label_dict.json \
	--with_char "no" \
	--char_len 5 \
	--predefined_vocab_size 50000 \
	--corpus_vocab_path porn/clean_data/textcnn/char_id.txt \
	--if_add_unlabeled_distillation "no" \
	--distillation_type "structure" \
	--tokenizer_type "full_bpe"