...
from data_generator import nlm_utils
...
#         print(left_index, "==left index==")
		left_index_mapping = {}
		for index, item in enumerate(left_index):
			left_index_mapping[index] = item
		document_cumsum_len = list(accumulate([len(document[index]) for index in left_index]))
		if document_cumsum_len[-1] >= target_seq_length:
			doc_splitted_index = first(range(len(document_cumsum_len)), condition = lambda index: document_cumsum_len[index]-target_seq_length >= 0)
		else:
			doc_splitted_index = len(document_cumsum_len) - 1
			
		if left_index_mapping[doc_splitted_index]-left_index_mapping[0] >= 1:
			a_end = random.sample(left_index[1:doc_splitted_index+1], 1)[0]
		else:
			a_end = left_index_mapping[0] + 1
 
		tokens_a = []
		for j in range(left_index_mapping[0], a_end):
			left_index.remove(j)
			tokens_a.extend(document[j])
		tokens_b = []
		
		if left_index_mapping[doc_splitted_index] == left_index_mapping[0] or rng.random() < 0.5:
			is_random_next = True
			...
			for j in range(a_end, left_index_mapping[doc_splitted_index]+1):
				left_index.remove(j)
...
def read_file(input_files, tokenizer, max_seq_length, ngram=2):
		...
		with tf.gfile.GFile(input_file, "r") as reader:
			while True:
				...
				if tokens and valid_flag:
					...
					all_documents_ngram_count = nlm_utils.nram_counts(tokens, 
													all_documents_ngram_count, 
													ngram)

					bg_hist_sets = nlm_utils.ngram_hist(tokens, bg_hist_sets, ngram)
	...
	return document_cnt, all_documents_ngram_count, bg_hist_sets
...
def create_instances_chunk_from_document(all_documents, document_index_chunk, 
		...
		continuations, gamma, ngram_scheme, D1, D2, D3p, 
		N1_lookup, N2_lookup, N3p_lookup, hist_freqs_cumsum, hist_freq_cumsum_token):
	...
	for document_index in document_index_chunk:
		instances = create_instances_from_document(
							...
							masked_lm_prob, max_predictions_per_seq,
							rng, num_of_documents,
							continuations, gamma, ngram_scheme,
							D1, D2, D3p, N1_lookup, N2_lookup, N3p_lookup, 
							hist_freqs_cumsum, 
							hist_freq_cumsum_token)
...
def multi_process(input_files, tokenizer,
	...
	[num_of_documents, 
	all_documents_ngram_count, 
	bg_hist_sets] = read_file(input_files, 
							tokenizer, 
							max_seq_length, 
							ngram=ngram)

	all_documents_ngram_fdist = nlm_utils.ngram_fdist(all_documents_ngram_count)
	continuations = nlm_utils.build_continuations(all_documents_ngram_count)
	[D1, D2, D3p, 
	N1_lookup, 
	N2_lookup, N3p_lookup] = nlm_utils.estimate_modkn_discounts(all_documents_ngram_count)

	bg_hist_counts = dict([(k, len(bg_hist_sets[k])) for k in bg_hist_sets])
	total_hists = sum(bg_hist_counts.values())
	hist_freqs = [(bg_hist_counts[token] /
					   float(total_hists),token) for token in bg_hist_counts]
	hist_freqs_cumsum = np.cumsum([item[0] for item in hist_freqs])
	hist_freq_cumsum_token = [item[1] for item in hist_freqs]
	...
	for chunk_id, chunk_key in enumerate(chunks):
		...
		create_instances_chunk_from_document(all_documents_shared, chunks[chunk_key], 
					...
					continuations, gamma, ngram_scheme, D1, D2, D3p, 
					N1_lookup, N2_lookup, N3p_lookup, 
	...
	# 				continuations, gamma, ngram_scheme, D1, D2, D3p, 
	# 				N1_lookup, N2_lookup, N3p_lookup, 
	# 				hist_freqs_cumsum, hist_freq_cumsum_token)) # apply_async
	# pool.close()
	# pool.join()
...
def create_masked_lm_predictions(tokens, masked_lm_prob,
	...
	for index_set in cand_indexes:
		...
		for index in index_set:
			...
			masked_token = nlm_utils.nlm_distribution(tokens[index], continuations, gamma, 
									ngram_scheme,
									D1, D2, D3p, N1_lookup, N2_lookup, N3p_lookup, 
									hist_freqs_cumsum, 
									hist_freq_cumsum_token)
			print(tokens[index], continuations['total'][tuple(tokens[index])],
				continuations['distinct'][tuple(tokens[index])], masked_token)
			# if rng.random() < 0.8:
			# 	masked_token = "[MASK]"
			# else:
