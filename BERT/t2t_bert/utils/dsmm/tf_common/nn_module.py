...
from utils.bimpm import rnn_model
# from utils.hard_attention import hard_attention_utils
...
def textbicurnn(x, num_units, cell_type, sequence_length, 
        num_layers=1, mask_zero=False, scope_name="bidirection_cudnn_rnn", 
        reuse=False, dropout_rate=0.3):

    batch_size = tf.shape(x)[0]
    input_size = tf.shape(x)[-1]

    with tf.variable_scope(scope_name+"_cubirnn_init", reuse=reuse): 
        rnn = rnn_model.BiCudnnRNN(num_units, batch_size, 
                            input_size,
        ...
        f_rep = res[0][:, :, 0:num_units]
        b_rep = res[0][:, :, num_units:2*num_units]
        outputs = res[0]

        return outputs

def textcurnn(x, num_units, cell_type, sequence_length, 
        num_layers=1, mask_zero=False, scope_name="cudnn_rnn", 
        reuse=False, dropout_rate=0.3):

    batch_size = tf.shape(x)[0]
    input_size = tf.shape(x)[-1]

    with tf.variable_scope(scope_name+"_curnn_init", reuse=reuse): 
        rnn = rnn_model.CudnnRNN(num_units, batch_size, 
                            input_size,
                            num_layers=num_layers, 
                            dropout=dropout_rate, 
                            kernel=cell_type)
...
def textrnn(x, num_units, cell_type, sequence_length, 
        num_layers=1, mask_zero=False, scope_name="textrnn", reuse=False):
    for i in range(num_layers):
...
def encode(x, method, params, input_dim,
    ...
    for m in method.split("+"):
        ...
        elif method == "textcurnn":
            dim_b = params["rnn_num_units"]
            z = textcurnn(x, num_units=params["rnn_num_units"], cell_type=params["rnn_cell_type"], num_layers=params["rnn_num_layers"],
                          sequence_length=sequence_length, 
                          mask_zero=mask_zero, scope_name=scope_name, 
                          reuse=reuse,
                          dropout_rate=dropout_rate)
            out_list.append(z)
            params["encode_dim"] += dim_b
        elif method == "textbicurnn": 
            dim_b = params["rnn_num_units"] * 2
            z = textbicurnn(x, num_units=params["rnn_num_units"], cell_type=params["rnn_cell_type"], num_layers=params["rnn_num_layers"],
                          sequence_length=sequence_length, 
                          mask_zero=mask_zero, scope_name=scope_name, 
                          reuse=reuse,
                          dropout_rate=dropout_rate)
