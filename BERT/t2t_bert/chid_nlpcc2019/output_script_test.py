...
def get_context_pair(resp, l):
    ...
    for index in range(len(valid_resp['pred_label'])):
        ...
        position_token = position_tokens[int(position)][1]
        if label == 1:
            score = 1 - score
...
valid_keys = ['input_ids', 'label_weights', 
			...
			'segment_ids']
for index, content in enumerate(dev_total_output):
    total_resp = []
    start = time.time()
    for t in content:
        tmp = {}
    ...
    resp = format_socre_matrix(total_resp, score_merge='max')
