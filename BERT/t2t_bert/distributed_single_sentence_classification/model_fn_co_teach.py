...
from model_io import model_io
import copy
from label_noise import co_teach
...
def model_fn_builder(
	...
	def model_fn(features, labels, mode):
		...
		if mode == tf.estimator.ModeKeys.TRAIN:
			coteach_model_config = copy.deepcopy(model_config)  
			coteach_model_config.scope = new_model_config.scope + '_co_teach'
			coteach_model = model_api(coteach_model_config, features, labels,
							mode, target, reuse=model_reuse)

			if coteach_model_config.fix_lm == True:
				co_teach_scope = coteach_model_config.scope + "_finetuning"
			else:
				co_teach_scope = coteach_model_config.scope

			with tf.variable_scope(co_teach_scope, reuse=model_reuse):
				(coteach_loss, 
				coteach_per_example_loss, 
				coteach_logits) = classifier.classifier(coteach_model_config,
												coteach_model.get_pooled_output(),
												num_labels,
												label_ids,
												dropout_prob)

			ori_loss, co_teach_loss = co_teach.co_teach(per_example_loss, 
								coteach_per_example_loss,
								tf.train.get_or_create_global_step(), 
								opt_config.num_train_steps,
								opt_config.get("epoch_k", 3),
								opt_config.epoch,
								model_config.get('ratio_rate', 0.8))

			loss = ori_loss + co_teach_loss

			co_teach_tvars = model_io_fn.get_params(coteach_model_config.scope, 
										not_storage_params=not_storage_params)
			tvars.extend(co_teach_tvars)
