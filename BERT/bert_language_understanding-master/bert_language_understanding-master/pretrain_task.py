# -*- coding: utf-8 -*-
import jieba
from data_util_hdf5 import _CLS, PAD_ID,UNK_ID,MASK_ID,_PAD,_UNK,_MASK,create_or_load_vocabulary,CLS_ID
import random
import re
import numpy as np
import os
...
def mask_language_model(source_file,target_file,index2word,max_allow_sentence_length=10,test_mode=False,process_num=10):
    ...
    for i, line in enumerate(source_lines):
        ...
        sentence_list = [line.split("__label__")[0]]
        for j,sentence in enumerate(sentence_list):
            ...
            if sentence_length>max_allow_sentence_length-1: 
                length =  max_allow_sentence_length - 1
            ...
            if sentence_length >= max_allow_sentence_length: # string list is longer then sentence_length
                string_list=string_list[0:max_allow_sentence_length-1]
            else: # ignore short sentence currently, temporary
                string_list = string_list + [_PAD] * (max_allow_sentence_length-1 - length)
            # the training data generator chooses 15% of tokens at random,e.g., in the sentence my dog is hairy it chooses hairy.
            assert len(string_list) == max_allow_sentence_length - 1
    ...
    print(np.array(X_mask_lm).shape)
...
if __name__ == "__main__":
    ...
    train, valid, test=mask_language_model(source_file,data_path,index2word,max_allow_sentence_length=10)#print("X_mask_lm:",X_mask_lm)
