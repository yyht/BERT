...
class BertCNNModel:
    ...
    def loss_lm(self,l2_lambda=3e-5):
        # input: `logits` and `labels` must have the same shape `[batch_size, num_classes]`
        # output: A 1-D `Tensor` of length `batch_size` of the same type as `logits` with the softmax cross entropy loss.
        # let `x = logits`, `z = labels`.  The logistic loss is:z * -log(sigmoid(x)) + (1 - z) * -log(1 - sigmoid(x))
        # y_mask_lm_onehot=tf.one_hot(self.y_mask_lm,self.vocab_size)
        print("#loss_lm.y_mask_lm_onehot:",self.y_mask_lm,";logits_lm:",self.logits_lm)
        losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits_lm,
                            labels=self.y_mask_lm)
