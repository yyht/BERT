...
tf.app.flags.DEFINE_integer("max_allow_sentence_length",30,"max length of allowed sentence for masked language model")
...
tf.app.flags.DEFINE_integer("sequence_length_lm",30,"max sentence length for masked language model")
...
tf.app.flags.DEFINE_integer("gpu",0,"number of cpu process")
tf.app.flags.DEFINE_string("word2vec_vocab_path","/data","number of cpu process")
print(FLAGS.word2vec_vocab_path)
import os
os.environ["CUDA_VISIBLE_DEVICES"] = str(FLAGS.gpu)
try:
    with open(FLAGS.word2vec_vocab_path, "r") as frobj:
        cnt = 0
        for line in frobj:
            word = line.strip()
            if len(word) >= 1:
...
def main(_):
    print(FLAGS.max_allow_sentence_length, "===max len===")
    ...
    print(FLAGS.test_mode, "===test mode===", vocab_word2index["<CLS>"])
    ...
    with tf.Session(config=gpu_config) as sess:
        ...
        monitor = int(number_of_training_data/float(batch_size))
        iteration=0
        score_best=-100
